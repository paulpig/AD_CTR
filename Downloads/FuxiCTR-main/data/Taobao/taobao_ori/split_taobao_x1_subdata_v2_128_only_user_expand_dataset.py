...
# concat train_df
extra_pos_df = all_df[(all_df['clk']=='1') & (all_df['time_stamp'] < '2017-05-10')]
# filter outside train_df
extra_pos_df = extra_pos_df[extra_pos_df['userid'].isin(user_voc) & extra_pos_df['adgroup_id'].isin(item_voc)]
train_df_ext = pd.concat([extra_pos_df, train_df], ignore_index=True)
#pdb.set_trace()
...
for idx, row in train_df_ext.iterrows():
    if idx % 10000 == 0:
...
#pdb.set_trace()
# user-based data
assert train_df_ext.shape[0] == len(click_history_users_col)
train_df = train_df_ext
...
train_df.to_csv("train_norm_only_user_add_ext_pos.csv", index=False)
test_df.to_csv("test_norm_only_user_add_ext_pos.csv", index=False)
