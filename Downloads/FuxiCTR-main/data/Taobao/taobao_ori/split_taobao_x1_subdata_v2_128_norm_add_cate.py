...
for idx, row in train_df.iterrows():
    ...
    # cate-based sequences
    click_cate_history = click_cate_sequence_queue[user_id]
    if len(click_cate_history) > sequence_max_len:
        click_cate_history = click_cate_history[-sequence_max_len:]
        click_cate_sequence_queue[user_id] = click_cate_history
    click_history_cate_col.append("^".join(click_cate_history)) #每个<user, item> pair, add click history_col
    if click == "1": # click sequences;
        click_cate_sequence_queue[user_id].append(cate_id)
...
train_df.loc[:, "click_cate_sequence"] = click_history_cate_col
...
test_df.loc[:, "click_cate_sequence"] = test_df["userid"].map(lambda x: "^".join(click_cate_sequence_queue[x][-sequence_max_len:]))
#pdb.set_trace()
...
train_df.to_csv("train_sub_norm_add_cate.csv", index=False)
test_df.to_csv("test_sub_norm_add_cate.csv", index=False)
