    ...
    for idx, row in user_features_df.iterrows():
        ...
        for a_idx, a_type in enumerate(user_at_type):
            ...
            if pd.isnull(user_at):
                ...
                #if at_key not in at2id[a_idx]:
                #    at2id[a_idx][at_key] = len(at2id[a_idx]) + 1
                    #itemAt2id[at_item_key] = len(itemAt2id) + 1
                #user_list.append(user2id[user_key])

                at2id = feat2id_dct[a_type]
                if user_id_v not in userid_atlist:
                    #userid_atlist[user_id_v] = [at2id[a_idx][at_key]]
                    userid_atlist[user_id_v] = [at2id[user_at]]
                else:
                    #userid_atlist[user_id_v].append(at2id[a_idx][at_key])
                    userid_atlist[user_id_v].append(at2id[user_at])
