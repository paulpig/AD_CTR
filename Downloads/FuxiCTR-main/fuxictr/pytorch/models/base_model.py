...
class BaseModel(nn.Module):
    def __init__(self, 
        ...
        # aux_loss: KL, CL and others
        if 'aux_loss_type' in self.kwargs_params:
            self.aux_loss_type = self.kwargs_params['aux_loss_type']
    ...
    def add_loss(self, inputs, reduction="mean"):
        ...
        if 'aux_loss_type' in self.kwargs_params:
            if self.aux_loss_type == 'CL':
                # pdb.set_trace()
                loss += float(self.kwargs_params['aux_loss_w']) * return_dict["aux_loss"]
                        ...
                        if self._embedding_initializer is not None:
                            try:
                                if v.padding_idx is not None:
                                    # the last index is padding_idx, 为什么不初始化padding呢?
    ...
    def on_batch_end(self, batch, logs={}, pre_model=None, encoder_model_pre=None, reindex_user_ids=None):
        self._total_batches += 1
        if (batch + 1) % self._every_x_batches == 0 or (batch + 1) % self._batches_per_epoch == 0:
            # forward 
            # if pre_model != None and encoder_model_pre != None and reindex_user_ids != None:
            #     pre_model = self.reload_pretrain_model()
            #     pre_model.forward_embeddings(save_model_name=encoder_model_pre)
            #     # model.embedding_layer.embedding_layer['userid_cp'] = model_pre.user_embeddings[reindex_user_ids]
            #     self.embedding_layer.other_emb_layer = pre_model.user_embeddings[reindex_user_ids]
            #     # self.embedding_layer.embedding_layer["userid"].weight =  torch.nn.Parameter(pre_model.user_embeddings[reindex_user_ids])
            #     # pdb.set_trace()
    ...
    def fit_generator(self, data_generator, epochs=1, validation_data=None,
                      verbose=0, max_gradient_norm=10., pre_model=None, encoder_model_pre=None, reindex_user_ids=None, **kwargs):
        """
        ...
        for epoch in range(epochs):
            epoch_loss = self.train_one_epoch(data_generator, epoch, pre_model=pre_model, encoder_model_pre=encoder_model_pre, reindex_user_ids=reindex_user_ids)
    ...
    def train_one_epoch(self, data_generator, epoch, pre_model=None, encoder_model_pre=None, reindex_user_ids=None):
        # pdb.set_trace()
        epoch_loss = 0
        self.train()
        ...
        for batch_index, batch_data in enumerate(batch_iterator):
            ...
            self.on_batch_end(batch_index, pre_model=pre_model, encoder_model_pre=encoder_model_pre, reindex_user_ids=reindex_user_ids)
            if self._stop_training:
                break
