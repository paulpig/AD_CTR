...
class HyperGraphCustomBipartiteDisenGATVAEV3(nn.Module):
    def __init__(self,
                #  adj_user_mat,
                 gpu=-1,
                 graph_layer=1,
                 ...
                 add_norm=False,
                 channels=2):
        super(HyperGraphCustomBipartiteDisenGATVAEV3, self).__init__()
        ...
        # path_origin = "../data/Taobao/taobao_ori/user_id_Item_id_net_v2_ori_num_replace_customer_true_only_pos_v2.npz" #构建customer-user hypergraph
        ...
        path_origin = "../data/Taobao/taobao_ori/user_id_Item_id_net_v2_ori_num_true_ad_user_v1.npz" #构建customer-user hypergraph
        ...
        # path = "../data/Taobao/taobao_ori/user_id_Item_id_pre_adj_mat_filter_top_30_ori_num_replace_customer_true_bipartile_only_pos_true_v2.npz" #超图卷积处理后的user-user graph, 只保留每个用户前N个相似的用户; customer-user graph;
        ...
        path = "../data/Taobao/taobao_ori/user_id_Item_id_pre_adj_mat_filter_top_30_ori_num_replace_customer_true_bipartile_only_pos_true_ad_user_v1.npz" #超图卷积处理后的user-user graph, 只保留每个用户前N个相似的用户; customer-user graph;
        # 之前存放训练数据的路径;
        ...
        # self.inter_path = "../data/Taobao/taobao_ori/train_data_dict_only_pos_v2.npz" #训练数据存放的路径;
        self.inter_path = "../data/Taobao/taobao_ori/train_data_dict_only_pos_ad_user_v1.npz" #训练数据存放的路径;
        ...
        self.path_dict_path = "../data/Taobao/taobao_ori/userItmeDict_v2_ori_num_true_ad_user_expand_v1.pk" # 存在ID与index的字典, <user_id, index>, <item_id, index>
        # self.path_dict_path_ori_user_item_2_ids = "../data/Taobao/taobao_ori/userItmeDict_v2_ori_num_true_ad_user_v1.pk" # 存在ID与index的字典, <user_id, index>, <item_id, index>
        ...
        with open(self.path_dict_path, 'rb') as r_f:
            self.userAt2id = pickle.load(r_f)
        ...
        with open('../data/Taobao/taobao_ori/user_attri2id_ad_user_dict.pk', "rb") as f:
            self.at2id = pickle.load(f) # list of {key_name: id}
        # with open('../data/Taobao/taobao_ori/userid2attids_dict.pk', "rb") as f:
        with open('../data/Taobao/taobao_ori/userid2attids_ad_user_dict.pk', "rb") as f:
    ...
    def bpr_loss_bipartite(self, users, pos, neg, model_name="disengcn"):
        """
        input:
            users: (bs), user_id
            pos: (bs), item_id
        ...
        if model_name == "lightgcn":
            self.all_user_embeddings = self.computer(self.Graph, torch.cat([self.embedding_user.weight, self.embedding_item.weight], dim=0)) #(user_num + item+num, dim)
        ...
        for (batch_i,
         (batch_users,
          ...
          batch_users_attris)) in enumerate(self.minibatch(target_users,
            ...
            batch_user_attri_emb = self.user_attri_emb
            # pdb.set_trace()
            # 用户属性特征交互;
            ...
            if add_vae_loss:
                # add vae loss
                # recon_w, kl_w = 5.e-3, 1.e-1
                # recon_w, kl_w = 1., 1.e-1
                # recon_w, kl_w = 1., 1.
                # recon_w, kl_w = 1.0, 0.1
                # recon_w, kl_w = 0.1, 0.1
                ...
                all_user_embeddings_detach =  self.all_user_embeddings
                batch_input_user_emb = all_user_embeddings_detach[batch_users.long()] #GNN输出的表征, 梯度不回传, 只更新VAE模块;
                batch_user_at_emb = self.user_preference_sample[batch_users.long()]
                # batch_user_at_emb = self.user_preference_sample
                batch_user_z, batch_user_mu, batch_user_var = self.user_z[batch_users.long()], self.user_mu[batch_users.long()], self.user_var[batch_users.long()]
            ...
            # u_c_pair_loss += reg_loss*self.weight_decay
            ...
            # print(f"loss{loss_cpu:.3f}--u_c_pair_loss{u_c_pair_loss_cpu: .3f}--KL_loss{recon_loss_total_cpu: .3f}--recon_loss{kl_loss_total_cpu: .3f}--{time()-start_time}")
    ...
    def forward_embeddings(self, save_model_name="disengcn", use_vae=False, is_save=False, is_train=True):
        ...
        if save_model_name == "embedding":
            ...
            all_embeddings = self.computer(self.Graph, torch.cat([self.embedding_user.weight, self.embedding_item.weight]))
