...
class HyperGraphCustomBipartiteDisen(nn.Module):
    def __init__(self,
                #  adj_user_mat,
                 gpu=-1,
                 graph_layer=1,
                 ...
                 weight_decay=1e-5):
        super(HyperGraphCustomBipartiteDisen, self).__init__()
        ...
        self.path_train_data = "../data/Taobao/taobao_ori/user_item_train_data_top30_ori_num_replace_customer_true_bipartile.npz" #训练数据, <user, in_session_positive_users>, <user, out_session_negative_users>; customer-user graph;
        ...
        self.path_pretrain_user_emb_path = "../data/Taobao/taobao_ori/user_emb_hypergraph_dim10_final_output_epoch_20_ori_num_top30_customer_user_true_bipartile_disen.h5" # 训练20个epoch, 保存输出的user representation;
    ...
    def disentangleComputer(self, adjacency_list, user_emb, item_emb):
        ...
        for i in range(self.layers):
            ...
            for i in range(self.channels):  
                z = torch.matmul(all_emb, self.weight_list[i]) + self.bias_list[i]
            ...
            for sub_adjacency in adjacency_list:
                ...
                if sub_adjacency.indices().shape[1] == 0:
                    ...
                    batch_all_output_emb = self.disentangledLearningOnLayer(sub_adjacency, batch_emb_list, all_channel_emb_list, n=count)
    ...
    def bpr_loss_bipartite(self, users, pos, neg):
        """
        input:
            users: (bs)
            pos: (bs)
        ...
        users_emb_lookup, items_emb_lookup = torch.split(all_user_embeddings, [self.user_voc_len, self.item_voc_len])
        # all_user_embeddings = self.computer(self.Graph, self.embedding_user.weight)
        # users_emb = self.embedding_user(users.long()) #(bs, dim)
        # pos_emb   = self.embedding_user(pos.long()) #(bs, max_len, dim)
        # neg_emb   = self.embedding_user(neg.long()) #(bs, max_len, dim)

        users_emb = items_emb_lookup[users.long()]
        pos_emb = users_emb_lookup[pos.long()]
        neg_emb = users_emb_lookup[neg.long()]
    ...
    def cal_loss(self, split=True):
        ...
        try: #目前存放的数据不正确, 先调试正确;
            ...
            for adj_user in train_data_sub_list:
                ...
                target_user_id_list, pos_users_list, neg_users_list = self.train_sample_gen_bipartite(adj_user, start_index)
