...
class HyperGraphCustomBipartiteDisenGATCL(nn.Module):
    def __init__(self,
                #  adj_user_mat,
                 gpu=-1,
                 graph_layer=1,
                 ...
                 weight_decay=1e-5):
        super(HyperGraphCustomBipartiteDisenGATCL, self).__init__()
        ...
        self.path_pretrain_user_emb_path = "../data/Taobao/taobao_ori/user_emb_hypergraph_dim10_final_output_epoch_20_ori_num_top30_customer_user_true_bipartile_disen_cl.h5" # 训练20个epoch, 保存输出的user representation;
    ...
    def disentangleComputer(self, adjacency_list, user_emb, item_emb):
        ...
        for i in range(self.layers):
            ...
            # add softmax
            update_A_val = F.softmax(update_A_val, dim=0)
    ...
    def bpr_loss_bipartite(self, items, pos, neg):
        """
        input:
            users: (bs), item_id
            pos: (bs), user_id
        ...
        item_emb = items_emb_lookup_lightgcn[items.long()]
        pos_emb = users_emb_lookup_lightgcn[pos.long()]
        neg_emb = users_emb_lookup_lightgcn[neg.long()]


        item_emb_d = items_emb_lookup_disen[items.long()]
        # pos_emb_d = users_emb_lookup_disen[pos.long()]
        # neg_emb_d = users_emb_lookup_disen[neg.long()]

        # add cl loss based items
        cl_loss = self.loss_contrastive(item_emb, item_emb_d,temp_value=0.1)
        ...
        pos_scores= torch.sum(item_emb*pos_emb_pooling, dim=1) #(bs)
        neg_scores= torch.sum(item_emb*neg_emb_pooling, dim=1)
