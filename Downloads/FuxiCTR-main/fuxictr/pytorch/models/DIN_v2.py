...
class DIN_SUB(BaseModel):
    def __init__(self, 
        ...
        # multi-interests
        self.inter_num = 3
        # user_multi_interest_matrix = nn.Embedding(self.feature_map.feature_specs["userid"]["vocab_size"], 
        #                                           self.embedding_dim * self.inter_num, 
        #                                           padding_idx= self.feature_map.feature_specs["userid"].get("padding_idx", None)).to(device=self.device)

        # item_multi_interest_matrix = nn.Embedding(self.feature_map.feature_specs["adgroup_id"]["vocab_size"], 
        #                                           self.embedding_dim * self.inter_num, 
        #                                           padding_idx= self.feature_map.feature_specs["adgroup_id"].get("padding_idx", None)).to(device=self.device)
        ...
        # share item embedding and user embedding
        user_multi_interest_matrix = self.embedding_layer.embedding_layer["userid"]
        item_multi_interest_matrix = self.embedding_layer.embedding_layer["adgroup_id"]
        self.multi_interest_matrix = [item_multi_interest_matrix, user_multi_interest_matrix]
        # add MLP
        self.multi_inter_mlp = MLP_Layer(input_dim=self.embedding_dim,
                             output_dim=self.inter_num * self.embedding_dim,
                             hidden_units=[],
    ...
    def forward(self, inputs):
        ...
        # din_sequence_field: [item_seq_list, user_seq_list]
        # din_target_field: [target_item, target_user]
        # pdb.set_trace()
        self.feature_list = [] # for loss
        if mask_label == 1:
            for idx, (target_field, sequence_field) in enumerate(zip(self.din_target_field, 
                                                                    self.din_sequence_field)):
                # target_emb = self.concat_embedding(target_field, feature_emb_dict) #(bs, dim)

                target_emb = feature_emb_dict[target_field] #(bs, dim)
                if type(sequence_field) != tuple:
                    sequence_emb = feature_emb_dict[sequence_field] # (bs, sl, dim)
                feature_index = self.feature_map.feature_specs[target_field]["index"]
                # 
                # inp = X[:, feature_index].float().view(-1, 1)
                inp = X[:, feature_index].long().view(-1, 1) # 是不是出现大量的NONE导致的?
                # pdb.set_trace()
                embedding_vec = self.multi_interest_matrix[idx](inp) #(bs, dim)
                embedding_vec = self.multi_inter_mlp(embedding_vec) #(bs, inter_num*dim)

                embedding_vec = embedding_vec.squeeze(1)
                embedding_vec_list = torch.split(embedding_vec, self.embedding_dim, dim=1)

                embedding_multi_vec = torch.stack(embedding_vec_list, dim=1) #(bs, inter_num, dim)
                # pdb.set_trace()
                multi_inter_sim = torch.bmm(embedding_multi_vec, sequence_emb.permute(0, 2, 1)) #(bs, inter_num, sl)
                
                seq_field = np.array([sequence_field]).flatten()[0] # pick a sequence field
                padding_idx = self.feature_map.feature_specs[seq_field]['padding_idx']
                # mask: (bs, sl), sl=128;
                mask = (X[:, self.feature_map.feature_specs[seq_field]["index"]].long() != padding_idx).float() # (bs, sl), 'index' means the positions of columns;
                mask = mask.unsqueeze(1).repeat(1, self.inter_num, 1) #(bs, inter_num, sl)
                # multi_inter_sim = multi_inter_sim.masked_fill((1.0-mask).byte(), float('-inf'))
                multi_inter_sim = multi_inter_sim.masked_fill((1.0-mask).bool(), -1e8)

                multi_inter_sim = F.softmax(multi_inter_sim, dim=-1) #(bs, inter_num, sl)

                multi_inter_sim = multi_inter_sim * mask

                output_multi_inter_emb = (sequence_emb.unsqueeze(1) * multi_inter_sim.unsqueeze(-1)).sum(-2) #(bs, inter_num, dim)

                self.feature_list.append(output_multi_inter_emb)

                # pdb.set_trace()
                pooling_emb = self.attention_layers[idx](target_emb, output_multi_inter_emb) #(bs, dim)
                # pdb.set_trace()
                # pooling_emb = output_multi_inter_emb[:,0,:] #(bs, dim), test for first embedding;
                # pooling_emb = sequence_emb[:,0,:] #(bs, dim), test for first embedding;
                # attention merge
                # pdb.set_trace()
                feature_emb_dict[sequence_field] = pooling_emb # 基本上都是0;
                # feature_emb_dict[sequence_field] = target_emb # test
                # feature_emb_dict[field] = field_emb

        # 1. MMD loss for similar representations
