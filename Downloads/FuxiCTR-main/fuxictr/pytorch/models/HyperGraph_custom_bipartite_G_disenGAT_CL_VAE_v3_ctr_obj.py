...
class HyperGraphCustomBipartiteDisenGATVAEV3CTRObj(nn.Module):
    def __init__(self,
                #  adj_user_mat,
                 gpu=-1,
                 graph_layer=1,
                 ...
                 add_norm=False,
                 channels=2):
        super(HyperGraphCustomBipartiteDisenGATVAEV3CTRObj, self).__init__()
        ...
        path_origin = "../data/Taobao/taobao_ori/u_c_pos_mat.npz" #构建customer-user hypergraph
        ...
        path = "../data/Taobao/taobao_ori/u_c_pos_mat_pre_norm.npz" #超图卷积处理后的user-user graph, 只保留每个用户前N个相似的用户; customer-user graph;

        self.inter_path = "../data/Taobao/taobao_ori/train_data_u_clist.npz" #训练数据存放的路径;
        
        self.path_dict_path = "../data/Taobao/taobao_ori/u2id_c2id.pk" # 存在ID与index的字典, <user_id, index>, <item_id, index>        
        
        self.path_pretrain_user_emb_path = "../data/Taobao/taobao_ori/u_c_pos_emb.h5" # 训练20个epoch, 保存输出的user representation;
        # 这个路径不要变;
    ...
    def bpr_loss_bipartite(self, t_customers, pos_u_list, neg_u_list, pos_uat_list, neg_uat_list, model_name="disengcn"):
        """
        input:
            users: (bs), user_id
            pos: (bs), item_id
