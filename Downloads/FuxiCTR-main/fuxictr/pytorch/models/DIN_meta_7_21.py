    ...
    def forward(self, inputs):
        ...
        if mask_label == 1:
            for idx, (target_field, sequence_field) in enumerate(zip(self.din_target_field, 
                ...
                if self.use_meta:
                    ...
                    if idx == 0:
                        ...
                        if self.use_pretrain and not self.concat_other_feat:
                            ...
                            meta_mapping_bias = self.meta_net_flat(user_embedding_hypergraph) #(bs, dim)

                            # add gate 
                            gate = nn.functional.sigmoid(self.gate_layer(torch.cat([pooling_emb, meta_mapping_bias], dim=-1)))
                            pooling_emb = gate * pooling_emb + (1. - gate) * meta_mapping_bias
                ...
                # else:
                #     pdb.set_trace()
                #     meta_mapping =  self.meta_net_v2(sequence_emb, mask) #(bs, dim, dim)
                #     pooling_emb = torch.bmm(target_emb.unsqueeze(1), meta_mapping).squeeze(1) #(bs, dim)

                # add cl
                # sequence_emb_aug = nn.functional.dropout(sequence_emb, 0.8)
                # meta_mapping_aug =  self.meta_net(sequence_emb_aug, mask) #(bs, dim, dim)
                # pooling_emb_aug = torch.bmm(target_emb.unsqueeze(1), meta_mapping_aug).squeeze(1) #(bs, dim)

                # if idx == 0:
                #     self.cl_loss = self.loss_contrastive(pooling_emb, pooling_emb_aug)
                # else:
                #     self.cl_loss += self.loss_contrastive(pooling_emb, pooling_emb_aug)
