...
class HyperGraphCustomBipartiteDisenGATVAE(nn.Module):
    def __init__(self,
                #  adj_user_mat,
                 gpu=-1,
                 graph_layer=1,
                 ...
                 weight_decay=1e-5):
        super(HyperGraphCustomBipartiteDisenGATVAE, self).__init__()
        ...
        # pdb.set_trace() # np.array(list(userid_atlist.values())).shape,  aaa = list(userid_atlist.keys())
    ...
    def disentangleComputer(self, adjacency_list, user_emb, item_emb):
        ...
        for i in range(self.layers):
            ...
            # self.after_mlp.append(torch.cat(all_channel_emb_list, dim=-1))
            # add user attributes, 合并用户属性, 之后再生成多个表征;
            # self.user_attri_emb: (n_user, 7, dim)
        ...
        # self.after_mlp =  np.sum(self.after_mlp, 0) / (self.layers)
    ...
    def disentangledLearningOnLayerGAT(self, adj, item_emb_list, all_emb_list, start_index=0):
        ...
        for l in range(self.iterations):
            ...
            for i in range(self.channels):
                ...
                v_list = torch.sum(all_emb_row * torch.tanh(all_emb_col), dim=1) #(M), 是不是考虑mask u-u和i-i矩阵呢?
    ...
    def bpr_loss_bipartite(self, users, pos, neg, model_name="disengcn"):
        ...
        if model_name == "lightgcn":
            ...
            all_user_embeddings = self.disentangleComputer(self.Graph, self.embedding_user.weight, self.embedding_item.weight) #(user_num + item+num, dim)
    ...
    def bpr_loss_bipartite_add_user_attris(self, users, pos, neg, model_name="disengcn", add_vae_loss=True):
        """
        input:
            users: (bs), user_id
            pos: (bs), item_id
    ...
    def cal_loss_v2(self, add_cl=False, model_name="disengcn"):
        ...
        # target_user_attris = torch.Tensor(train_data[3]).long().to(self.device) # padding
        
        # shuffle traindata
        # target_users, pos_users, pos_len, neg_users, neg_len = self.shuffle(target_users, pos_users, pos_len, neg_users, neg_len)
        # target_users, pos_users, neg_users, target_user_attris = self.shuffle(target_users, pos_users, neg_users, target_user_attris)
         ...
         (batch_users,
          ...
          batch_neg_users)) in enumerate(self.minibatch(target_users,
            ...
            if add_cl == True:
                ...
                loss, reg_loss, cl_loss = self.bpr_loss_bipartite_add_user_attris(batch_users, batch_pos_users, batch_neg_users, model_name=model_name)
    ...
    def save_embeddings(self, save_model_name="disengcn"):
        ...
        if save_model_name == "embedding":
            ...
            all_embeddings = self.disentangleComputer(self.Graph, self.embedding_user.weight, self.embedding_item.weight).detach().cpu().numpy() #(user_num + item+num, dim)
