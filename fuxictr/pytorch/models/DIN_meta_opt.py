...
class DIN_Meta(BaseModel):
    def __init__(self, 
        ...
        input_dim = (feature_map.num_fields + 1)* embedding_dim
        # self.crossnet = CrossNet(input_dim, len(dnn_hidden_units))
        # self.fc = nn.Linear(input_dim + embedding_dim, 1) # [cross_part, dnn_part] -> logit
        # self.output_activation = self.get_output_activation(task)
        
        self.dnn_ori = MLP_Layer(input_dim=input_dim,
                             output_dim=1,
    ...
    def forward(self, inputs):
        ...
        feature_emb_dict = self.embedding_layer(X, add_bn=False)
        ...
        # feature_emb_dict["hyper_emb"] = self.embedding_layer.bn(convert_emb_hyper)
        feature_emb_dict["hyper_emb"] = convert_emb_hyper

        feature_emb = self.embedding_layer.dict2tensor(feature_emb_dict)

        # add batch normalize
        # feature_emb = self.bn(feature_emb)

        # pdb.set_trace()
        # cross_out = self.crossnet(feature_emb.flatten(start_dim=1)) # (b, dim1)

        # dnn_feat = self.dnn(feature_emb.flatten(start_dim=1))

        # y_pred = nn.functional.sigmoid(self.fc(torch.cat([cross_out, dnn_feat], dim=-1)))
