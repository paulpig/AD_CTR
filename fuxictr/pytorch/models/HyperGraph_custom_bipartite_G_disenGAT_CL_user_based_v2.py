...
class HyperGraphCustomBipartiteDisenGATCLUserV2(nn.Module):
    def __init__(self,
                #  adj_user_mat,
                 gpu=-1,
                 graph_layer=1,
                 ...
                 add_norm=False,
                 channels=2):
        super(HyperGraphCustomBipartiteDisenGATCLUserV2, self).__init__()
        ...
        # adj_mat_t = self.adj_mat.T
        # adj_mat_t.getrow(20).getcol(25)
        # self.ui_mat.toarray()
        # np.allclose(self.ui_mat, self.ui_mat.T, rtol=1e-8, atol=1e-8)
        # np.all(np.abs(a-a.T) < tol)
    ...
    def generate_train_data_for_train(self, voc_len):
        ...
        for t_user, pos_items_list in zip(self.target_user_list, self.pos_items_list):
            ...
            while True:
                ...
                neg_idx = np.random.randint(1, voc_len)
                # pdb.set_trace()
                # neg_idx = np.random.choice(self.occur_item_voc_set, 1, replace=False)[0]
    ...
    def disentangleComputer(self, adjacency_list, user_emb, item_emb, mode='train'):
        ...
        for i in range(self.layers):
            ...
            for l in range(self.iterations_valid):
                ...
                # pdb.set_trace()
                # update_A_val = torch.stack(pre_val_list, dim=0) + update_A_val # 删除的效果更好;
    ...
    def computer(self, adjacency_list, embedding, add_mlp=False):
        ...
        for i in range(self.layers):
            ...
            if add_mlp == True:
                item_embeddings = torch.matmul(item_embeddings, self.weight_lightgcn) + self.bias_lightgcn #每层的参数设置不同吗?
                # z = self.weight_list[i](all_emb)
            ...
            for sub_adjacency in adjacency_list:
                ...
                # e_idx = s_idx + sub_adjacency.shape[0]
                # item_embeddings_sub = item_embeddings[s_idx:e_idx]
                # s_idx = e_idx
                # add l2 normalize
                # item_embeddings_sub = F.normalize(item_embeddings_sub, dim=1)
                # item_embeddings_sub = F.normalize(item_embeddings_sub, dim=1)
                # item_embeddings_sub = torch.nn.functional.normalize(item_embeddings_sub, p=2, dim=1)
    ...
    def cal_loss_v2(self, add_cl=False, model_name="disengcn"):
        ...
        # pdb.set_trace() # check训练数据没有问题; 构建的(u+i, u+i) matrix没有问题;
        # shuffle traindata
    ...
    def save_embeddings(self, save_model_name="disengcn"):
        """
        ...
        if save_model_name == "embedding":
            ...
            all_embeddings = self.embedding_user.weight.cpu().detach().numpy()
        elif save_model_name == "lightgcn":
            # 2. original item embeddings
            # all_embeddings = self.computer(self.Graph, torch.cat([self.embedding_user.weight, self.embedding_item.weight])).detach().cpu().numpy()
            # all_embeddings = self.computer(self.Graph, torch.cat([self.embedding_user.weight, self.embedding_item.weight]), add_mlp=True).detach().cpu().numpy()
            all_embeddings = self.computer(self.Graph, torch.cat([self.embedding_user.weight, self.embedding_item.weight]), add_mlp=False).detach().cpu().numpy()
        elif save_model_name == "disengcn":
            all_embeddings = self.disentangleComputer(self.Graph, self.embedding_user.weight, self.embedding_item.weight, mode="predict").detach().cpu().numpy() #(user_num + item+num, dim)
