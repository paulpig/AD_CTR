...
class DIN_Meta_HIS(BaseModel):
    def __init__(self, 
                 ...
                 **kwargs):
        super(DIN_Meta_HIS, self).__init__(feature_map, 
                                  model_id=model_id, 
                                  gpu=gpu, 
        ...
        self.dnn_ori = MLP_Layer(input_dim=(feature_map.num_fields + 0)* embedding_dim,
                             output_dim=1,
        ...
        # self.weight_scalar_linear = torch.nn.Linear(embedding_dim, 1, True)
        self.weight_scalar_linear = torch.nn.Linear(embedding_dim, embedding_dim, True)
    ...
    def forward(self, inputs):
        ...
        if self.use_meta and self.use_pretrain:
            ...
            # user_embedding_hypergraph = self.embedding_layer.other_emb_layer[inp] #消融实验;
            # self.embedding_layer.embedding_layer['userx id'](inp)
        ...
        if 'his_clk' in feature_emb_dict:
            his_clk_emb = feature_emb_dict['his_clk']
        ...
        # feature_emb_dict["hyper_emb"] = F.normalize(user_embedding_hypergraph, dim=1)
        convert_emb_hyper = self.pretrain_emb_mlp(user_embedding_hypergraph)
        feature_emb_dict["hyper_emb"] = convert_emb_hyper

        # # calculate the weight of hyper_emb
        # hyper_scalar = nn.ReLU()(self.weight_scalar_linear(his_clk_emb)) #(bs, 1)

        # feature_emb_dict["hyper_emb"] = convert_emb_hyper * hyper_scalar

        # # convert_emb_hyper = convert_emb_hyper * hyper_scalar
