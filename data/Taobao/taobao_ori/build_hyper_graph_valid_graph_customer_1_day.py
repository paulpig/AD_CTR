...
train_df = df[(df['time_stamp'] < '2017-05-13') & (df['time_stamp'] >= '2017-05-12')] #筛选近三天的数据;
#train_df = df[(df['time_stamp'] < '2017-05-13')] #筛选近三天的数据;
test_df = df[df['time_stamp'] >= '2017-05-13']
behavior_df = df[df['time_stamp'] < '2017-05-12'] # 构建超图的df;
...
feat2id_dct = {}
#root_path = "../../taobao_x1_sub_base_only_user_min_1_pretrain_custom_user_only_pos_no_pretrain/"
root_path = "../../taobao_x1_sub_base_only_user_min_1_pretrain_custom_user_only_pos_no_pretrain_1_day/"
for i_type in item_at_type + user_at_type:
        ...
        if pd.isnull(item_at) or item_at not in customer_voc_set:
            ...
            for user_type in user_at_type:
                ...
                if pd.isnull(user_at) or user_at not in user_voc_set:
                    ...
                    try:
                        userAt2id = feat2id_dct[user_type]
                        itemAt2id = feat2id_dct[item_type]
                    except:
                        raise NotImplementedError('Can not find feature id.')

                    user_attributes.append(userAt2id[user_at])
                    item_attributes.append(itemAt2id[item_at])

                    # add dict
                    if userAt2id[user_at] not in interact_items_d:
                        interact_items_d[userAt2id[user_at]] = [itemAt2id[item_at]]
                    else:
                        interact_items_d[userAt2id[user_at]].append(itemAt2id[item_at])
                    trainDataSize += 1
...
#m_item = len(item_voc)
m_item = len(customer_voc) + 1
m_user = len(user_voc) + 1
...
# save dict
#dict_path ="./train_data_dict_only_pos_v2.npz"
...
#pdb.set_trace()
# 3. 根据日志构建hypergraph
#sp.save_npz('./user_id_Item_id_net_v2_ori_num_replace_customer_true_only_pos_v2.npz', UserItemNet)
sp.save_npz('./u_c_pos_mat_new_idx_1_day.npz', UserItemNet)

#path = "userItmeDict_v2_ori_num_replace_customer_true_only_pos_v2.pk"
#path = "u2id_c2id_news_idx.pk"
