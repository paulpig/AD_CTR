            ...
            for user_type in user_at_type:
                ...
                if pd.isnull(user_at) or user_at not in user_voc_set:
                    ...
                    if userAt2id[at_key] not in interact_items_d:
                        interact_items_d[userAt2id[at_key]] = [itemAt2id[at_item_key]] # userid: list of items
                    else:
...
# save dict
dict_path ="./train_data_dict_only_pos.npz"
...
#pdb.set_trace()
# 3. 根据日志构建hypergraph
sp.save_npz('./user_id_Item_id_net_v2_ori_num_replace_customer_true_only_pos.npz', UserItemNet)
